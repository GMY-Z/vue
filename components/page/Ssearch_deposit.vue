  ...
  export default {
    ...
    methods: {
      search () {
        ...
        this.$http.post('/api/user/Ssearch_deposit').then(res => {
          console.log("ccccccccccccccc222c");
          this.$message({
            message: '查询成功！',
            type: 'success'
          })
          ...
          this.cols = [];
          for (var j = 0; j < res.data.cols_list.length; j++) {
         
            var obj = {}
            obj.label = res.data.cols_list[j].label
            obj.prop = res.data.cols_list[j].prop
          ...
          this.tableData = [];
          for (var i = 0; i < res.data.each_row.length; i++) {
            this.tableData.push(res.data.each_row[i])
          }
      ...
      search_by(card_id, value){
        const self = this;	
        this.$http.post('/api/user/Ssearch_deposit_by',{name: card_id, time: value}).then(res => {
          this.cols = []
