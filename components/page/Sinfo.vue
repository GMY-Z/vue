<template>
	<div>
        ...
        <el-breadcrumb-item><i class="el-icon-user"></i>个人中心</el-breadcrumb-item>
      </el-breadcrumb>
		</div>
	...
	export default {
		...
		methods:{
			getUserData() {
				...
				self.$http.get('/api/user/getStuff',{name: username}).then(function(response) {
					console.log(response);
					let result = response.data[0];
					self.form.number = result.number;
          ...
          type: 'warning'
					...
					const self = this;
				self.$http.post('/api/user/changeStuff',self.form)
				.then((response) => {
					console.log(response);
